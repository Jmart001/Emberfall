...
function drawResources(now) {
  for (const r of ROCKS) {
    const X = r.x * TILE + 16,
      Y = r.y * TILE + 18,
      depleted = r.depletedUntil && tickCount < r.depletedUntil;
    ctx.fillStyle = depleted ? '#1a1c19' : '#242823';
    ctx.beginPath();
    ctx.moveTo(X - 13, Y + 9);
    ctx.lineTo(X - 9, Y - 8);
    ctx.lineTo(X + 1, Y - 14);
    ...
    ctx.fill();
    if (depleted) {
      // Mined out: a bare, hollowed silhouette with no ore glints, so it's obvious at a glance.
  ...
  const FX = FURNACE.x * TILE + 16,
    FY = FURNACE.y * TILE + 17,
    glow = 0.6 + 0.3 * Math.sin((now || 0) / 260);
  ctx.fillStyle = '#3a2f28';
  ctx.beginPath();
  ctx.moveTo(FX - 12, FY + 11);
  ctx.lineTo(FX - 12, FY - 9);
  ctx.lineTo(FX - 4, FY - 15);
  ctx.lineTo(FX + 12, FY - 15);
  ctx.lineTo(FX + 12, FY + 11);
  ctx.closePath();
  ctx.fill();
  ctx.fillStyle = '#241c17';
  ctx.fillRect(FX - 7, FY - 2, 14, 12);
  ctx.fillStyle = `rgba(255,${Math.round(120 + glow * 60)},40,${glow})`;
  ctx.fillRect(FX - 5, FY, 10, 8);
  ctx.fillStyle = `rgba(255,210,140,${glow * 0.5})`;
  ctx.beginPath();
  ctx.arc(FX, FY + 4, 9, 0, 7);
  ctx.fill();
}
...
function actor(x, y, color, name, kind = 'human', opts = {}) {
  ...
  if (kind === 'goblin' || kind === 'warden' || kind === 'bogling') {
  ...
  } else if (kind === 'wolf' || kind === 'direWolf') {
    const isDire = kind === 'direWolf',
      s = isDire ? 1.15 : 1; // dire wolves render a notch larger than regular wolves
    ctx.strokeStyle = isDire ? '#1c2022' : '#2c3235';
    ctx.lineWidth = isDire ? 3 : 2;
    ctx.fillStyle = color;
    ctx.beginPath();
    ctx.ellipse(X - 1, Y, 14 * s, 9 * s, -0.08, 0, 7);
    ctx.fill();
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(X + 5, Y - 5);
    ...
    ctx.fillStyle = isDire ? '#d64a3a' : '#e0b951';
    ctx.fillRect(X + 12, Y - 10, 2, 2);
    ctx.fillStyle = isDire ? '#22181a' : '#333a3c';
  ...
  } else if (kind === 'skeleton' || kind === 'guardian' || kind === 'unboundConstruct') {
    ctx.strokeStyle = color;
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(X, Y - 5);
    ...
    ctx.fillStyle =
      opts.legs === 'bronzeLegs' ? '#a3703e' : opts.legs === 'ironLegs' ? '#6d7175' : '#302116';
    ctx.fillRect(X - 7, Y + 6, 5, 10);
    ctx.fillRect(X + 2, Y + 6, 5, 10);
    ctx.fillStyle =
      opts.armor === 'bronzeArmor'
        ? '#b0763f'
    ...
    ctx.fill();
    if (opts.helmet) {
      ctx.fillStyle = opts.helmet === 'bronzeHelmet' ? '#b0763f' : '#747c7d';
      ctx.beginPath();
    ...
    if (!opts.helmet && hash(name.length, name.charCodeAt(0)) % 3 === 0) {
    ...
    if (opts.player && opts.shield) {
      ctx.fillStyle = opts.shield === 'ironShield' ? '#747c7d' : '#8f7445';
      ctx.strokeStyle = opts.shield === 'ironShield' ? '#aeb7b8' : '#d0b16a';
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(X - 10, Y + 1, 7, 0, 7);
      ctx.fill();
      ctx.stroke();
      ctx.fillStyle = opts.shield === 'ironShield' ? '#c8ced0' : '#c5a15d';
...
function drawNpcSpeech() {
  for (const n of NPCS) {
    ...
    const X = n.drawX * TILE + 16,
      tailY = n.drawY * TILE - 40, // bottom edge of the bubble, where the tail starts
      maxWidth = 210;
    ctx.font = '10px Arial';
    ctx.textAlign = 'center';
    // Word-wrap into multiple lines instead of clipping to a fixed single-line box —
    // the old box width was capped at 190px but fillText never wraps on its own, so
    // any longer line just overflowed past the bubble's edges.
    const words = n.speech.split(' ');
    const lines = [];
    let line = '';
    for (const word of words) {
      const test = line ? line + ' ' + word : word;
      if (line && ctx.measureText(test).width > maxWidth - 16) {
        lines.push(line);
        line = word;
      } else line = test;
    }
    ...
    const lineHeight = 12,
      ...
      boxW = Math.min(maxWidth, textW + 16),
      boxH = lines.length * lineHeight + 8,
      boxTop = tailY - boxH;
    ctx.fillStyle = '#11120fea';
    ctx.fillRect(X - boxW / 2, boxTop, boxW, boxH);
    ctx.strokeStyle = '#8b7445';
    ctx.strokeRect(X - boxW / 2 + 0.5, boxTop + 0.5, boxW - 1, boxH - 1);
    ctx.fillStyle = '#eee2be';
    lines.forEach((l, i) => ctx.fillText(l, X, boxTop + lineHeight * (i + 1) - 1));
    ctx.fillStyle = '#11120fea';
    ctx.beginPath();
    ctx.moveTo(X - 4, tailY);
    ctx.lineTo(X, tailY + 6);
...
function render(now) {
  ...
  monsters
    ...
    .forEach((m) => {
      ...
      if ((m.kind === 'warden' || m.kind === 'unboundConstruct') && m.enraged) {
...
function updateCoordHud() {
  const el = coordHudEl || (coordHudEl = document.getElementById('coordHud'));
  if (!el) return;
  const region = inDungeon(player.x, player.y) ? 'Ashen Barrow' : regionNameFor(player.x, player.y),
    text = `<b>Tile</b> ${player.x}, ${player.y}\n<b>Hover</b> ${hover.x}, ${hover.y}\n${region}`;
  if (text !== lastCoordHud) {
    lastCoordHud = text;
    el.innerHTML = text;
  }
}
// OSRS-style XP drop: a small badge in the top-right corner for every XP gain, driven by
// syncLevels() diffing player.xp each tick, so every skill is covered without touching
// each individual XP-granting call site.
function showXpDrop(skill, amount) {
  const container = document.getElementById('xpDrops');
  if (!container) return;
  const el = document.createElement('div');
  el.className = 'xpDrop';
  el.innerHTML = `<span class="xpIcon">${(typeof SKILL_ICONS !== 'undefined' && SKILL_ICONS[skill]) || '⭐'}</span>+${amount} ${skill}`;
