function skillTick() {
  ...
  if (skilling.type === 'fish') {
    ...
    player.drawX = 174;
    player.drawY = 44;
    camera.x = Math.max(0, 174 * TILE - canvas.clientWidth / 2);
    camera.y = Math.max(0, 44 * TILE - canvas.clientHeight / 2);
    player.homeTeleportAt = Date.now();
    effects.push({ x: 174, y: 44, value: 'HOME', color: '#8bcdf0', at: performance.now(), xp: 0 });
    ...
    const recipe = skilling.recipe,
      station = recipe.station === 'furnace' ? FURNACE : FORGE;
    if (!adjacent(player, station)) {
      skilling = null;
      return message(`You move away from the ${station.name.toLowerCase()}.`, 'bad');
    }
    if (!canAdd(recipe.makes)) {
      skilling = null;
      return message('Your backpack is full.', 'bad');
    }
    const missing = Object.entries(recipe.needs).find(([id, q]) => invCount(id) < q);
    if (missing) {
      skilling = null;
      return message(`You are out of ${ITEMS[missing[0]].name.toLowerCase()}.`, 'bad');
    }
    // EF1: keeps smelting/forging each tick, same as fishing/mining/chopping, instead of
    // stopping after a single bar/item and forcing a re-click.
    skilling.next = tickCount + 4;
    lastActionAt = performance.now();
    actionDuration = TICK_MS * 4;
    for (const [id, q] of Object.entries(recipe.needs)) add(id, -q);
    ...
    message(`You create ${ITEMS[recipe.makes].name}.`, 'good');
    if (skilling.remaining != null && --skilling.remaining <= 0) skilling = null;
    renderPanel();
    save();
...
// OSRS-style smithing interface: an icon grid per station (furnace = bars, anvil = gear)
// with a quantity selector, instead of a plain list of "Smelt X — level Y" buttons.
let smithAmount = 'all',
  smithStation = null;
function openFurnace() {
  smithStation = 'furnace';
  renderSmithStation();
}
function openAnvil() {
  smithStation = 'anvil';
...
function renderSmithStation() {
  const kind = smithStation,
    title = kind === 'furnace' ? 'Cinderforge Furnace' : 'Cinderforge Anvil',
    hint =
      kind === 'furnace'
        ? 'Smelt ore into bars here. Bring your bars to the anvil to forge equipment.'
        : 'Forge bars into weapons and armour here.',
    recipes = RECIPES.filter((r) => r.station === kind),
    qbar = [
      [1, '1'],
      [5, '5'],
      [10, '10'],
      ...
      .map(
        ([v, label]) =>
          `<button class="qbtn ${smithAmount === v ? 'on' : ''}" data-mq="${v}">${label}</button>`,
      )
      .join(''),
    grid = recipes
      .map((r) => {
        const maxByMats = Math.min(
          ...Object.entries(r.needs).map(([id, q]) => Math.floor(invCount(id) / q)),
        );
        const canLevel = level('Smithing') >= r.level;
        const need = Object.entries(r.needs)
          .map(([id, q]) => `${q} ${ITEMS[id].name}`)
          .join(', ');
        return `<div class="slot bankslot ${canLevel && maxByMats > 0 ? '' : 'locked'}" data-recipe="${r.id}" title="${r.name} — level ${r.level}, needs ${need}${maxByMats > 0 ? ` (have enough for ${maxByMats})` : ''}"><img class="ic" src="assets/items/${r.makes}.png" alt="" draggable="false"><span class="shopPrice">Lv${r.level}</span></div>`;
      })
      .join('');
  modalBody.innerHTML =
    `<h2>${title}</h2><p class="hint">${hint}</p>` +
    `<div class="bankBar"><span class="hint">Make</span>${qbar}</div>` +
    `<div class="bankGrid">${grid}</div>` +
    `<div class="bankActions"><button class="choice" data-act="close">Close</button></div>`;
  modal.classList.remove('hidden');
  modalBody
    .querySelectorAll('[data-mq]')
    .forEach((b) => (b.onclick = () => setSmithAmount(b.dataset.mq === 'x' || b.dataset.mq === 'all' ? b.dataset.mq : +b.dataset.mq)));
  modalBody.querySelectorAll('[data-recipe]').forEach((s) => {
    const recipe = recipes.find((r) => r.id === s.dataset.recipe);
    s.onclick = () => startSmith(recipe);
  });
  modalBody.querySelector('[data-act="close"]').onclick = closeModal;
}
function setSmithAmount(v) {
  if (v === 'x') {
...
function startSmith(recipe) {
  ...
  closeModal();
  combat = null;
  skilling = { type: 'smith', recipe, next: tickCount + 4, remaining: smithAmount === 'all' ? null : smithAmount };
  lastActionAt = performance.now();
  actionDuration = TICK_MS * 4;
  message(`You begin to ${recipe.name.toLowerCase()}.`, 'game');
}
...
function adminMenu() {
  showModal('Admin Panel', 'God mode active: invincible, always running.', [
    ...
    ['Restart a quest', adminQuestResetMenu],
    ['Disable admin mode', () => setAdmin(false)],
    ['Close', closeModal],
  ]);
}
function adminQuestResetMenu() {
  const quests = [
    { label: 'A Wanderer in Greenrest', reset: () => (player.story = { q: 0, step: 0 }) },
    { label: 'Shadows Over Pineholt', reset: () => (player.story = { q: 1, step: 0 }) },
    {
      label: 'Beneath the Ashen Barrow',
      reset: () => {
        player.story = { q: 2, step: 0 };
        add('key', -invCount('key'));
        add('relic', -invCount('relic'));
        player.barrowRunStartedAt = 0;
        player.barrowPotential = 0;
        resetBarrowRun();
      },
    },
    { label: 'The Boar Hunt', reset: () => (player.sideQuests.boarHunt = { step: 0, kills: 0 }) },
    {
      label: 'Silk and Cinders',
      reset: () => (player.sideQuests.silkAndCinders = { step: 0, kills: 0 }),
    },
    { label: 'The Broken Road', reset: () => (player.sideQuests.brokenRoad = { step: 0, kills: 0 }) },
    { label: 'Hearth and Home', reset: () => (player.sideQuests.hearthAndHome = { step: 0, kills: 0 }) },
    {
      label: 'A Cure for Sablemarsh',
      reset: () => (player.sideQuests.cureMirehaven = { step: 0, kills: 0 }),
    },
  ];
  const rows = quests.map((q) => [
    ...
    () => {
      ...
      updateUI();
      save();
      message(`Restarted quest: ${q.label}.`, 'good');
      adminMenu();
    },
    'danger',
  ]);
  rows.push(['Back', adminMenu]);
  showModal(
    'Restart a Quest',
    'Testing tool: resets the chosen quest back to its first step. Does not touch skills, gold, or unrelated inventory.',
    rows,
  );
}
...
function renderPanel() {
  ...
  if (activeTab === 'inventory') {
    ...
    // OSRS-style fixed grid: every skill is always visible at once, no scrolling regardless
    // of window size, unlike the old one-row-per-skill list.
    p.innerHTML =
      '<div class="title">SKILLS</div><div class="skillGrid">' +
      Object.keys(player.xp)
        .map((k) => {
          ...
          return `<div class="skillTile" data-skill="${k}" title="${k}: level ${lv}, ${player.xp[k]} XP. Click for the guide."><span class="sIcon">${SKILL_ICONS[k] || '⭐'}</span><span class="sLv">${lv}</span><i class="sFill" style="width:${pct}%"></i></div>`;
        })
        .join('') +
      '</div>';
    ...
    // OSRS-style full loadout: weapon, helmet, platebody, platelegs, shield, gloves, and a
    // charm slot reserved for bonus trinkets to be added later.
    const GEAR_SLOTS = [
      ['weapon', 'Weapon'],
      ['helmet', 'Helmet'],
      ['armor', 'Platebody'],
    ...
    const equipGrid = GEAR_SLOTS.map(([key, label]) => {
      const id = player.equipment[key];
      return `<div class="slot" data-equip="${key}" title="${id ? 'Remove ' + ITEMS[id].name : label + ' — empty'}">${id ? `<img class="ic" src="assets/items/${id}.png" alt="">` : '-'}</div>`;
    }).join('');
    const equipRows = GEAR_SLOTS.map(
      ([key, label]) =>
        `<div class="row"><span>${label}</span><b>${player.equipment[key] ? ITEMS[player.equipment[key]].name : 'None'}</b></div>`,
    ).join('');
    const combatStyleSection = `<div class="title combatTitle">COMBAT STYLE</div>${['accurate', 'aggressive', 'defensive'].map((s) => `<button class="choice ${player.combatStyle === s ? 'selected' : ''}" data-style="${s}">${s[0].toUpperCase() + s.slice(1)} - train ${s === 'accurate' ? 'Attack' : s === 'aggressive' ? 'Strength' : 'Defence'}</button>`).join('')}`;
    const spellOrAmmoSection = isMagic
      ? `<div class="title combatTitle">SPELLBOOK</div>${Object.entries(SPELLS).map(([id, sp]) => `<button class="choice ${player.selectedSpell === id ? 'selected' : ''}" data-spell="${id}" ${level('Magic') < sp.level ? 'disabled' : ''}><b>${sp.name}</b> - Magic ${sp.level}, ${sp.runes} rune${sp.runes === 1 ? '' : 's'}${sp.bind ? ', binds target' : sp.dmgBonus ? ', +' + sp.dmgBonus + ' max hit' : ''}</button>`).join('')}<div class="row"><span>Runes</span><b>${invCount('emberRune')} Ember</b></div><p class="hint">Attacks from up to ${weapon.range} tiles away.</p>`
      : isRanged
        ? `<div class="row"><span>Ammunition</span><b>${invCount('arrows')} arrows</b></div><p class="hint">The bow attacks from up to ${weapon.range} tiles away.</p>`
        : '';
    p.innerHTML = `<div class="title">EQUIPMENT</div><div class="grid">${equipGrid}</div>${!isMagic && !isRanged ? combatStyleSection : ''}${equipRows}${spellOrAmmoSection}<div class="row"><span>${isMagic ? 'Magic' : isRanged ? 'Ranged' : 'Attack'} bonus</span><b>+${isMagic ? weapon.magic || 0 : isRanged ? weapon.ranged || 0 : weapon.attack || 0}</b></div><div class="row"><span>Defence bonus</span><b>+${equipmentDefence()}</b></div><div class="row"><span>Max hit</span><b>${max}</b></div><div class="row"><span>Attack speed</span><b>${weapon.speed || 4} ticks</b></div>${weapon.spec ? `<div class="row"><span>Special</span><b>${weapon.spec === 'power' ? 'Crushing blow (+60% dmg, 50%)' : 'Flurry — double hit (50%)'}</b></div>` : ''}`;
    ...
    p.querySelectorAll('[data-pin]').forEach((b) => (b.onclick = () => setActiveQuest(b.dataset.pin)));
  }
}
...
function pinButtonHTML(key) {
  const on = player.activeQuestKey === key;
  return `<button class="questPinBtn ${on ? 'on' : ''}" data-pin="${key}">${on ? '★ Active quest — click to unpin' : 'Set as active'}</button>`;
}
function questsTabHTML() {
  ...
  const story = QUESTS.map((q, qi) => {
    ...
    return `<div class="qcard ${st.toLowerCase()}"><div class="qhead"><b>${q.name}</b><span class="qbadge ${st.toLowerCase()}">${st}</span></div>${q.steps
      ...
      .join('')}<p class="reward"><b>Reward:</b> ${q.reward}</p>${st === 'ACTIVE' ? pinButtonHTML('main') : ''}</div>`;
  }).join('');
...
function hearthQuestHTML() {
  ...
  return `<div class="title">${status} Hearth and Home</div><div class="questStep ${q.step === 2 ? 'done' : q.step === 1 ? 'current' : ''}">${q.step === 0 ? (available ? 'Speak with Tamsin at the Resting Stag in Pineholt.' : 'Help Captain Rowan secure Pineholt first.') : q.step === 1 ? `Gather feast ingredients: boar meat ${Math.min(1, invCount('rawMeat'))}/1, cabbage ${Math.min(1, invCount('cabbage'))}/1, wild herb ${Math.min(1, invCount('herb'))}/1${have ? ' — return to Tamsin.' : ''}` : 'Pineholt celebrated beneath a warm and crowded roof.'}</div><p><b>Reward:</b> 120 coins, 3 Pineholt stews, Cooking and Farming XP</p>${available && q.step < 2 ? pinButtonHTML('hearthAndHome') : ''}`;
}
    ...
    status =
      q.step === 0
        ...
        : q.step === 1
          ? '[ACTIVE]'
          : '[DONE]';
  return `<div class="title">${status} A Cure for Sablemarsh</div><div class="questStep ${q.step === 2 ? 'done' : q.step === 1 ? 'current' : ''}">${q.step === 0 ? (available ? 'Speak with Healer Sable in Sablemarsh — the marsh-fever is the Ember seeping up from below.' : 'Help Greenrest before travelling southwest.') : q.step === 1 ? `Gather Bog moss from Boglings: ${Math.min(4, invCount('bogMoss'))}/4${invCount('bogMoss') >= 4 ? ' — return to Sable.' : ''}` : 'Sablemarsh is free of the marsh-fever.'}</div><p><b>Reward:</b> Sablemarsh charm, 2 antidotes, 160 coins, Herblore and Defence XP</p>${available && q.step < 2 ? pinButtonHTML('cureMirehaven') : ''}`;
}
...
function barrowLoreHTML() {
  ...
  return (
    `<div class="title">ASHEN BARROW</div><div class="bestiaryRow known"><div><b>Dungeon clears</b><span>${player.barrowRuns} reward chest${player.barrowRuns === 1 ? '' : 's'} claimed</span></div><small>${barrowChestReady() ? 'Chest ready to claim' : 'Defeat the Warden to unlock the chest'}</small></div><div class="row"><span>Personal best</span><b>${player.barrowBestMs ? formatRunTime(player.barrowBestMs) : 'Not set'}</b></div><div class="row"><span>Last clear</span><b>${player.barrowLastMs ? formatRunTime(player.barrowLastMs) : 'Not set'}</b></div><div class="row"><span>Best reward potential</span><b>${player.barrowBestPotential || 0}%</b></div><p class="hint">Every chamber monster adds reward potential: guardians 10%, skeletons 8%, cave spiders 7%, and ash bats 5% each. Clearing all 8 guarded chambers reaches ~70% before you even reach the Warden.</p><div class="title">BARROW COLLECTION ${Object.values(player.collection.barrow).filter(Boolean).length}/3</div><div class="grid collectionGrid">${[
      ['cloak', 'wardenCloak'],
      ['guard', 'ashenGuard'],
...
function updateUI() {
  ...
  document.querySelector('#sidebar header small').textContent =
    `${player.story.q >= 3 ? 'CHAPTER 2' : 'CHAPTER 1'}  CMB ${combatLevel()}` +
    (admin ? '  ·  ⚙ ADMIN' : '');
  const s = player.story;
  questTracker.innerHTML =
    s.q >= QUESTS.length
      ? `<b>CHAPTER TWO</b>All ${QUESTS.length} quests complete.`
...
reset.onclick = openCharacterManager;
function timeAgo(ms) {
...
function openCharacterManager() {
  const curId = activeSlotId();
  const sorted = [...charSlots].sort((a, b) => b.lastPlayedAt - a.lastPlayedAt);
  const rows = sorted.map((s) => {
    const isActive = s.id === curId;
    const label = isActive
      ? `▶ ${s.name} (current) — saved ${timeAgo(s.lastPlayedAt)}`
      : `${s.name} — last played ${timeAgo(s.lastPlayedAt)}`;
    return isActive
      ? [label, closeModal, 'selected']
      : [label, () => switchToSlot(s.id)];
  });
  rows.push(['+ New character', createNewCharacter]);
  if (charSlots.length > 1) rows.push(['Delete a character...', openDeleteCharacterMenu]);
  rows.push(['Close', closeModal]);
  showModal(
    'Characters',
    `You have ${charSlots.length} character${charSlots.length === 1 ? '' : 's'}. Switching automatically saves your current progress first.`,
    rows,
  );
}
function openDeleteCharacterMenu() {
  const curId = activeSlotId();
  const rows = charSlots
    .filter((s) => s.id !== curId)
    .map((s) => [`Delete "${s.name}"`, () => deleteCharSlot(s.id), 'danger']);
  rows.push(['Back', openCharacterManager]);
  showModal('Delete a character', 'This cannot be undone. Your current character is not shown here.', rows);
}
